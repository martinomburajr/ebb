...
func TestGenerateRandomTree(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GenerateRandomTree(tt.args.length, tt.args.terminals, tt.args.nonTerminals)
...
func Test_calculateEnclosingParentheses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := calculateEnclosingParenthesesIndices(tt.args.bt, tt.args.nonTerminalIndex)
			if got != tt.want {
				t.Errorf("calculateEnclosingParenthesesIndices() got = %v, want %v", got, tt.want)
			}
			if got1 != tt.want1 {
				t.Errorf("calculateEnclosingParenthesesIndices() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
...
func TestRandom(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		fmt.Printf("\n\t\t\t################ EXPRESSION %d ################\n", i)
		tree := GenerateRandomTree(rand.Intn(length), BTreeTerminals, BTreeNonTerminals).
			ApplyOperatorOnTerminal('/', 'x').
			MutateNonTerminal(BTreeTerminals)

		tree = tree.AddSubTreeRandomly(GenerateRandomTree(rand.Intn(length), BTreeTerminals, BTreeNonTerminals)).
		...
		errors := tree.EvaluateTreeCorrectness()
		if len(errors) > 0 {
			t.Errorf("DeleteTerminal - FormatErrors\n%s", errors.ToString(tree))
		}
	}
}
