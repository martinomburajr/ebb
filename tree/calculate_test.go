...
func TestCalculateRolling(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := CalculateRolling(tt.args.bt, tt.args.xVal)
			if (err != nil) != tt.wantErr {
				t.Errorf("CalculateRolling() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("CalculateRolling() got = %v, want %v", got, tt.want)
			}
		})
	}
}
